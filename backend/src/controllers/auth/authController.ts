...
export async function loginWithPhone(request: Request, response: Response) {
  ...
  if (!userDocument) {
    response.status(404).json({ message: "Số điện thoại chưa được đăng ký. Vui lòng đăng ký tài khoản trước." });
    return;
  }

  const user = userDocument.toObject() as any;
  ...
  if (user.status === "pending") {
    response.status(403).json({ message: "Tài khoản chưa xác thực OTP. Vui lòng hoàn tất đăng ký trước." });
    return;
  }
...
export async function verifyLoginOtp(request: Request, response: Response) {
  ...
  if (!pending) {
    response.status(404).json({ message: "Không tìm thấy yêu cầu đăng nhập. Vui lòng gửi lại OTP." });
    return;
  }
  ...
  const userDocument = await UserModel.findOne({ phone }).lean();
  ...
  const user = userDocument as any;
  ...
  pendingLogins.delete(phone);
  response.status(200).json(toAuthPayload(user));
}
...
export async function registerWithPhone(request: Request, response: Response) {
  ...
  const district = String(request.body.district ?? "").trim();
  ...
  if (!city || !province || !district || !ward) {
  ...
  if (existingByEmail && (existingByEmail as any).phone !== phone) {
    response.status(409).json({ message: `Email đã được đăng ký với số ${maskPhone(String((existingByEmail as any).phone ?? ""))}.` });
    return;
  }
      ...
      if (!user) {
        ...
        try {
          const newUser = await UserModel.create({
            ...
            savedAddresses: ["Ho Chi Minh City"],
            avatar: `https://i.pravatar.cc/120?u=${code}`,
            createdAt: new Date()
          });
